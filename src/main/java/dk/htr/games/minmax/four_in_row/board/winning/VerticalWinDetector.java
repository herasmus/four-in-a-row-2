...
import static dk.htr.games.minmax.four_in_row.board.columns.four.FourRowColumns.COLUMN_4R_OOOO;
import static dk.htr.games.minmax.four_in_row.board.columns.four.FourRowColumns.COLUMN_4R_XXXX;
...
public class VerticalWinDetector implements WinDetector {
    ...
    public WinResult hasWinner(long board, int move, boolean blueDisc) throws GameException {
        ...
        if(blueDisc) {
            result = (column == COLUMN_4R_XXXX);
        } else {
            result = (column == COLUMN_4R_OOOO);
        }
